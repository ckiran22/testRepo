    ...
    return {
        ...
        controller: ['$scope', '$element', '$compile', function ($scope, $element, $compile) {
            ...
            angular.forEach($scope.modifyColumns, function (column, index) {
                var magreFieldColumn;
                var strMargeFieldTemplate = "";
                if (column.margeFieldOnSingle) {
                    magreFieldColumn = $scope.mapmodifyColumnsByName[column.margeFieldOnSingle];
                    strMargeFieldTemplate = "<span class = 'show-on-single'>" + ((magreFieldColumn) ? (magreFieldColumn.template) ? magreFieldColumn.template : (($scope.record[magreFieldColumn.name]) ? $scope.record[magreFieldColumn.name] : '') : "") + "</span>";
                }
                if (column.template)
                    strTemplate += '<td class="' + ((column.showAlways) ? 'showonmobile' : '') + ' ' + ((column.hideOnSingle) ? ' hide-on-single' : '') + '" onclick="unSelect(this);" id=' + column.label + '  ><b class="showHeader">' + column.label + '</b>' + column.template + ' ' + strMargeFieldTemplate + '</td>';
                    //strTemplate += '<td class="' + ((column.showAlways) ? 'showonmobile' : '') + ' ' + ((column.hideOnSingle) ? ' hide-on-single' : '') + '" onclick="unSelect(this);" id=' + column.label + ' data-th=' + column.label + '  >' + column.template + ' ' + strMargeFieldTemplate + '</td>';
                else
                    strTemplate += '<td class="' + ((column.showAlways) ? 'showonmobile' : '') + ' ' + ((column.hideOnSingle) ? ' hide-on-single' : '') + '" onclick="unSelect(this);" id=' + column.label + ' ><b class="showHeader">' + column.label + '</b>' + (($scope.record[column.name]) ? $scope.record[column.name] : '') + ' ' + strMargeFieldTemplate + '</td>';
                //strTemplate += '<td class="' + ((column.showAlways) ? 'showonmobile' : '') + ' ' + ((column.hideOnSingle) ? ' hide-on-single' : '') + '" onclick="unSelect(this);" id=' + column.label + ' data-th=' + column.label + '  >' + (($scope.record[column.name]) ? $scope.record[column.name] : '') + ' ' + strMargeFieldTemplate + '</td>';
            });
...
<div>\
	<div>\
		<table class="table list attachments responsive">\
			...
			<tbody>\
				<tr ng-show ="pagedRecords.length == 0" ><td colspan="100" class="showonmobile" >No Record Found</td></tr>\
				<tr ng-hide ="pagedRecords.length <= 0" ng-repeat="record in pagedRecords" enz-tablerow="record" columns="modifyColumns">\
				</tr>\
			</tbody>\
		</table>\
	</div>\
        ...
        controller: function ($scope, $attrs, $filter, $timeout, $compile, $element) {
            ...
            $scope.modifyColumnsName = function () {
                ...
                if ($scope.columns) {
                    ...
                    angular.forEach($scope.modifyColumns, function (column) {
                        ...
                        if (column.margeFieldOnSingle)
                            column.margeFieldOnSingle = column.margeFieldOnSingle.replace(".", "_");

                        if (!$scope.mapmodifyColumnsByName[column.name])
                            $scope.mapmodifyColumnsByName[column.name] = column;
                    })
                }
            }
            ...
            $scope.callGetDataFunction = function (page) {
                ...
                if ($scope.records.length < $scope.totalRecordsCount) {
                    if ($scope.keyword && ($scope.keyword.indexOf($scope.previousKeyword) != -1 && $scope.searchRecords.length == $scope.searchTotalRecordsCount)) {
